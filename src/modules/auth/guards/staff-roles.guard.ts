...
import { StaffRole } from '../../staff/entities/staff_role.entity';
...
export class StaffRolesGuard implements CanActivate {
  ...
  canActivate(ctx: ExecutionContext): boolean {
    const requiredStaffRoles = this.reflector.getAllAndOverride<
      StaffRole['role'][]
    >(STAFF_ROLES_KEY, [ctx.getHandler(), ctx.getClass()]);
    ...
    // user.staff is dynamically attached in JWT strategy
    ...
    const hasPermission = requiredStaffRoles.includes(staffRoleName);
